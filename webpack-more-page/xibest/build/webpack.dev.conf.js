...
//  https:true,//DevServer 默认使用 HTTP 协议服务，它也能通过 HTTPS 协议服务, 有些情况下你必须使用 HTTPS
    ...
    open: devConfig.autoOpenBrowser,//用于在 DevServer 启动且第一次构建完时自动用你系统上默认的浏览器去打开要开发的网页。 
    host: HOST || devConfig.host,//配置DevServer服务器监听的地址。如果你想要局域网中其他设备访问你本地的服务，可以在启动的时候带上--host 0.0.0.0.host的默认值是127.0.0.1即只有本地可以访问
    port: PORT || devConfig.port,//配置DevServer服务监听的端口，默认使用8080端口。如果8080端口已经被其他程序占有就使用8081....
    compress: true,//配置是否启用 gzip 压缩，默认为 false。
    overlay: devConfig.errorOverlay ? { warnings: false, errors: true }: false,//浏览器页面上显示错误
